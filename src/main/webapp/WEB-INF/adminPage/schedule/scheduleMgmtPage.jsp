...
<head>
  ...
  <script>
  	...
  	function theaterSearch(){
  		...
  		let url="http://192.168.50.87:9090${ctp}/ScheduleTheater.ad?startDate="+startDate+"&endDate="+endDate;
  	...
  	function MovieSearch(){
  		...
  		let url="http://192.168.50.87:9090${ctp}/MovieListPage.ad?startDate="+startDate;
		...
		function orderAdd(){
			...
			alert("nextOrderMinTime: "+nextOrderMinTime);
			...
			alert("nextOrderTime: "+nextOrderTime);
			...
			fileBox += '<input type="time" name="order" id="order'+cnt+'" min='+nextOrderMinTime+' value='+nextOrderTime+' class="form-control border mb-2 mr-2" style="width:70%"/>';
			fileBox += '<input type="button" value="삭제" onclick="deleteBox('+cnt+')" class="btn-danger btn-sm" style="float:left; width:20%"/>';
			fileBox += '</div>';
...
<div class="content">
	<div class="continer">
		<h3>영화 상영 일정표</h3>
		<input type="button" value="일정 추가" class="btn btn-success btn-sm border" data-toggle="modal" data-target="#ScheduleInsertModal"/>
  ...
  <div class="text-center">
    <table class="table table-bordered mt-2" style="height:700px;">
      	...
      	<c:forEach begin="1" end="${lastDay}" varStatus="st">
      	  ...
      	  ${st.count}<br/>
      	  	...
      	  	<c:forEach var ="vo" items="${vos}">
      	  		<c:if test="${fn:substring(vo.sDate,8,10) == st.count}">${vo.part}:${vo.count}건<br/></c:if>
      	  	</c:forEach>
      	  	</a>
      	  </td>
      ...
      <!-- Modal body -->
      	...
      	<div class="modal-body row justify-content-center">
	      	<div class="input-group mb-3">
    				...
    				<c:set var="today" value="${toYear}-${strToMonth}-${toDay}"></c:set>
			      <input type="date" class="form-control" name="startDate" id="startDate" min="${today}"/>
    				<div class="input-group-prepend">
				    	<span class="input-group-text">상영 종료일</span>
    				</div>
			      <input type="date" class="form-control" name="endtDate" id="endDate"/>
	  			</div>
	      	<div class="input-group mb-3">
    				<div class="input-group-prepend">
				    	<span class="input-group-text">상영관</span>
    				</div>
			      <input type="text" class="form-control" name="theaterName" id="theaterName" placeholder="상영관 조회를 하세요" readonly/>
			      ...
			      <input type="text" class="form-control" name="movieName" id="movieName" placeholder="영화 조회를 하세요" readonly/>
	      	...
	      	<div class="input-group mb-3">
			      ...
			      <input type="time" class="form-control" name="order" id="order1"/>
     	...
     	<!-- Modal footer --> 
			<div class="modal-footer">
	 			<input type="button" onclick="submitOk()" value="등록" class="btn btn-info"/>
